...
class Filters(object):
    def gabor(self,theta=0, gamma=1, sigma=2, lam=5.6,k=10):
...
#        edge_map = sp.ndimage.generic_filter(stim,selective_filter,size = gabor.shape)
    ...
    def run_thres(self):
        im = Image.open('megztinis_input.png').convert('L')
        ...
        for t in range(5): # loop over time
            print t,
            edge_map = np.zeros((len(sfs),len(oris))+mean.shape)
            #surface_map = np.zeros((len(oris),)+mean.shape)
            for sfno, sf in enumerate(sfs):
                for oi,ori in enumerate(oris):
                    gabor = Filters().gabor(theta=ori, sigma=2*sf,lam=5.6*sf,k=10)
                    ...
                    edge_map[sfno,oi] = edges
            mean = np.max(edge_map,1)
            mean = np.max(mean,0)
            #import pdb; pdb.set_trace()
        ...
        plt.axis('off')
        #plt.show()
        plt.savefig('megztinis.jpg', dpi=300, format='jpg',
